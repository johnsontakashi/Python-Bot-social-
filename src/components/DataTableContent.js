...
const DataTableContent = () => {
  ...
  const handleViewDetails = (row) => {
    alert(`Viewing details for: ${row.username}\n\n${row.text}`);
  };

  const handleEditRow = (row) => {
    alert(`Edit functionality for ${row.username} would open here`);
  };

  const handleDeleteRow = (row) => {
    if (window.confirm(`Delete post by ${row.username}?`)) {
      alert('Delete functionality would be implemented here');
    }
  };

  const handleOpenUrl = (row) => {
    // Extract URL from text or use placeholder
    const urlMatch = row.text.match(/https?:\/\/[^\s]+/);
    if (urlMatch) {
      window.open(urlMatch[0], '_blank');
    } else {
      alert('URL not found in post text');
    }
  };
